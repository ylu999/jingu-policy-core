...
export const p2Preconditions = (input: Input): Violation[] => {
  ...
  const hasMissingTool = failureSignals.some((signal) =>
    missingToolSignals.some((ms) => signal.toLowerCase().includes(ms.toLowerCase()))
  )

  if (!hasMissingTool) return violations

  // Determine which tools are likely missing based on signals
  for (const signal of failureSignals) {
    for (const tool of requiredTools) {
      if (
        signal.toLowerCase().includes(tool.toLowerCase()) ||
    ...
    if (
      signal.toLowerCase().includes("terraform_not_found") &&
