 ...
 */
export const p1ConstraintObedience = (input: Input): Violation[] => {
  const violations: Violation[] = []
  const constraints = input.task.constraints?.explicitConstraints ?? []

  if (constraints.length === 0) return violations
...
function violatesConstraint(
  action: { type: string; command?: string; intent: string },
  constraint: string
): boolean {
  ...
  if (normalizedConstraint === "terraformonly") {
    ...
    if (
      (cmd.includes("aws") || cmd.includes("gcloud") || cmd.includes("az ")) &&
  ...
  if (normalizedConstraint === "nowrite") {
    if (action.type === "write" || action.type === "command") return true
  }
...
function extractForbiddenKeyword(constraint: string): string | null {
  // "no_<keyword>" patterns
  const noMatch = constraint.toLowerCase().match(/^no[_\s](.+)$/)
  if (noMatch) return noMatch[1].replace(/[_\s]/g, "")
  return null
}
