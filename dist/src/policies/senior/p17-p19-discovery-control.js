...
exports.p17ExternalizeOnDiscovery = p17ExternalizeOnDiscovery;
exports.p18BlockerPrecedence = p18BlockerPrecedence;
exports.p19ContextBudget = p19ContextBudget;
// ---------------------------------------------------------------------------
// P17 — Externalize and Split on Discovery
//
...
// ---------------------------------------------------------------------------
function p17ExternalizeOnDiscovery(input) {
    const violations = [];
...
// ---------------------------------------------------------------------------
function p18BlockerPrecedence(input) {
    const violations = [];
    const { proposal, evidence, discoveries = [] } = input;
    // Detect 1: Unresolved blocker in discovery records while proposal proceeds
    const unresolvedBlockers = discoveries.filter(d => d.type === "blocker" && !d.resolvedInline);
    ...
    const hasMutatingActions = proposal.plannedActions.some(a => a.type === "write" || a.type === "command");
...
function p19ContextBudget(input) {
    const violations = [];
    ...
    const unresolvedNonBlockers = discoveries.filter(d => d.type !== "blocker" && !d.resolvedInline && !d.spawnSuggested);
...
function extractIntentAreas(intents) {
    const areas = new Set();
    const combined = intents.join(" ").toLowerCase();
    if (/test|spec|assert|coverage/.test(combined))
        areas.add("testing");
    if (/deploy|infra|terraform|kubernetes|docker|cloud/.test(combined))
        areas.add("infrastructure");
    if (/database|migration|schema|query|sql/.test(combined))
        areas.add("database");
    if (/auth|permission|role|token|credential/.test(combined))
        areas.add("auth");
    if (/lint|format|style|eslint|prettier/.test(combined))
        areas.add("style");
    if (/docs|readme|comment|documentation/.test(combined))
        areas.add("documentation");
    if (/ci|pipeline|workflow|github.action|build/.test(combined))
        areas.add("ci");
    if (/config|env|setting|environment/.test(combined))
        areas.add("config");
    if (/api|endpoint|route|handler/.test(combined))
        areas.add("api");
    if (/cache|redis|memcach/.test(combined))
        areas.add("cache");
    if (/network|firewall|dns|port/.test(combined))
        areas.add("network");
    if (/package|dependency|npm|yarn|install/.test(combined))
